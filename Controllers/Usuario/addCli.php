...
if($_POST){
		...
		if ($validarCorreo) {
			...
			Swal.fire({
				...
				icon: 'error',
				title: 'Los siento..!',
				text: 'El correo ingresado ya existe, intete digitar uno diferente',
			...
			Swal.fire({
				...
				icon: 'error',
				title: 'Los siento..!',
				text: 'El usuario ingresado ya existe, intete digitar uno diferente',
				showConfirmButton: false,
				timer: 3000
			  })
	
			  setTimeout(function(){window.location.href = '../../Views/templates/Usuario/addCli.php'}, 3000);
			</script>";
		}else{
		...
		if($agregar){
			header('Location:../../Views/templates/Cliente/add.php');
		}else{
...
if($_POST){
		...
		if ($validarCorreo) {
			...
			Swal.fire({
			  ...
			  setTimeout(function(){window.location.href = '../../src/index.html'}, 3000);
			</script>";
		}elseif ($validarUsuario) {
			echo "<script>
			Swal.fire({
			...
			$agregar=$modeloUsuario->agregar($idUsuario, $nombreUsuario, $apellidoUsuario, $rolUsuario, $documentoUsuario, $direccionUsuario, $correoUsuario, $passwordUsuario, $telefonoUsuario, $estadoUsuario, $foto);

		if($agregar){
		    echo "<script> window.location='../../view/cliente.php'; </script>";
		}else{
			echo "<script> window.location='../../src/index.html'; </script>";
		}
	}
}else{
	echo "<script> window.location='../../src/index.html'; </script>";
